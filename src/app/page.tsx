...
export default function Home() {
  ...
  return <div className="site-shell"><SiteHeader /><main id="main-content">
    ...
    <section className="section section-dark reveal" id="reviews"><div className="section-heading"><p className="eyebrow">Great reviews &amp; testimonials</p><WordReveal as="h2" className="display-section">Service people remember.</WordReveal></div><div className="testimonial-grid">{testimonials.slice(0, 3).map(review => <blockquote key={review.author} className="reveal-item"><span className="stars" aria-label="Rated 5 out of 5 stars">★★★★★</span><p>“{review.quote}”</p><cite>{review.author}</cite></blockquote>)}</div><Link className="button button-light" href="/realtor-reviews/">Read all testimonials <span className="btn-icon">↗</span></Link></section>
    ...
    <section className="section areas-section reveal" id="areas"><div className="section-heading"><p className="eyebrow">Serving the region</p><WordReveal as="h2" className="display-section">Local Realtors across Clear Lake NASA.</WordReveal></div><div className="area-grid">{serviceAreas.map((area, index) => <Link key={area} href={`/realtors-in-${areaSlug(area)}/`} className="reveal-item"><span className="area-num">{String(index + 1).padStart(2, '0')}</span>{area}<span>↗</span></Link>)}</div></section>
