...
export default function Home() {
  ...
  return <div className="site-shell"><IntroPreloader onDone={() => setStarted(true)} /><SiteHeader /><main>
    ...
    <section className="section intro-section reveal" id="welcome"><div className="section-heading"><p className="eyebrow">Welcome to 1st Texas Realtors</p><WordReveal as="h2" className="display-section">Clarity for buying, selling, and everything between.</WordReveal><p><strong>Family owned since 2004,</strong> we provide expert Realtors in Clear Lake, dedicated customer service, and real-time listings of homes for sale and rent.</p></div><div className="intro-copy"><p><strong>David Karstedt and wife Simone</strong> work as a team, bringing exceptional responsiveness, deep knowledge, strong negotiation skills, and patient service to each client.</p><div className="stats-band"><div className="stat"><strong>50+</strong><small>Years combined experience</small></div><div className="stat"><strong>Top 3%</strong><small>Realtor ranking</small></div><div className="stat"><strong>2004</strong><small>Family owned since</small></div></div><Link className="text-link" href="/realtor-reviews/">Read our testimonials <span>↗</span></Link></div></section>
    ...
    <section className="section section-dark reveal" id="reviews"><div className="section-heading"><p className="eyebrow">Great reviews &amp; testimonials</p><WordReveal as="h2" className="display-section">Service people remember.</WordReveal></div><div className="marquee reviews-marquee" aria-hidden="true"><div className="marquee-track">{[...testimonials, ...testimonials].map((review, i) => <span key={`${review.author}-${i}`} className="marquee-item"><span className="stars">★★★★★</span>{review.author} — “{review.quote.split(' ').slice(0, 6).join(' ')}…”</span>)}</div></div><div className="testimonial-grid">{testimonials.slice(0, 3).map(review => <blockquote key={review.author}><span className="stars" aria-label="Rated 5 out of 5 stars">★★★★★</span><p>“{review.quote}”</p><cite>{review.author}</cite></blockquote>)}</div><Link className="button button-light" href="/realtor-reviews/">Read all testimonials <span className="btn-icon">↗</span></Link></section>
    ...
    <VideoBand />
    <section className="press-row reveal" aria-label="Featured in"><span className="mono-label">Featured in</span><span className="press-item">Clear Lake Chamber of Commerce</span><span className="press-item">Houston Association of Realtors</span><span className="press-item">Texas Monthly</span><span className="press-item">NASA Area Community</span></section>
