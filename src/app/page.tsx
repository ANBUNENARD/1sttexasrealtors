...
export default function Home() {
  ...
  return <div className="site-shell"><SiteHeader /><main id="main-content">
    ...
    <section className="section section-dark nws-services reveal" id="services"><div className="section-heading"><p className="eyebrow">How we help</p><WordReveal as="h2" className="display-section">A better way forward.</WordReveal><p>From buying and selling to renting, staging, relocation, and commercial property — a full-service brokerage for every move.</p></div><div className="nws-card-grid">{services.map(([title, body, href, image, badge]) => <article className="nws-service-card reveal-item" key={title}><div className="nws-card-media"><img src={image} alt={title} loading="lazy" /><span className="nws-card-badge">{badge}</span><Link href={href} className="nws-card-arrow" aria-label={`${title} — learn more`}><span>→</span></Link></div><div className="nws-card-body"><h3>{title}</h3><p>{body}</p><Link className="nws-card-btn" href={href}>Learn more <span>→</span></Link></div></article>)}</div></section>
